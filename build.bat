...
:: FIX 2026-08-21: old logic picked the FIRST drive with an Include dir
:: (C: has SDK-only, no km\ntddk.h) and then failed. Now we require
:: km\ntddk.h to exist under some version before accepting a root,
:: searching D:, E:, F: first (same order as GPU driver build.bat).
set "WDK_ROOT="
set "WDK_VERSION="
for %%D in (D E F C G H) do (
    if "!WDK_ROOT!"=="" (
        if exist "%%D:\Program Files (x86)\Windows Kits\10\Include" (
            for /f "delims=" %%V in ('dir /b /ad "%%D:\Program Files (x86)\Windows Kits\10\Include" 2^>nul') do (
                if exist "%%D:\Program Files (x86)\Windows Kits\10\Include\%%V\km\ntddk.h" (
                    set "WDK_ROOT=%%D:\Program Files (x86)\Windows Kits\10"
                    set "WDK_VERSION=%%V"
                    echo Found WDK !WDK_VERSION! on %%D: drive
                )
            )
        )
        if "!WDK_ROOT!"=="" if exist "%%D:\Program Files\Windows Kits\10\Include" (
            for /f "delims=" %%V in ('dir /b /ad "%%D:\Program Files\Windows Kits\10\Include" 2^>nul') do (
                if exist "%%D:\Program Files\Windows Kits\10\Include\%%V\km\ntddk.h" (
...
if "!WDK_ROOT!"=="" (
    echo ERROR: Windows Kit with kernel headers not found
    echo Searched: D:, E:, F:, C:, G:, H: drives
    exit /b 1
)
