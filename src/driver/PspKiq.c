...
{
    KdPrint(("KIQ_HW: Programming KIQ registers (proxy=%d handle=%p)\n",
        (int)g_GpuProxyAvailable, g_GpuDriverHandle));

    if (!g_GpuProxyAvailable && g_GpuDriverHandle == NULL) {
        NTSTATUS proxyStatus = PspGpuProxyInit(devExt);
        if (!NT_SUCCESS(proxyStatus)) {
            KdPrint(("KIQ_HW: GPU proxy init failed 0x%08X\n", proxyStatus));
            return STATUS_DEVICE_NOT_READY;
        }
    }
    if (!g_GpuProxyAvailable) {
        KdPrint(("KIQ_HW: no GPU proxy — cannot program registers\n"));
        return STATUS_DEVICE_NOT_READY;
    }
...
NTSTATUS PspKiqInit(PDEVICE_EXTENSION devExt, ULONG64 ringPA, ULONG ringSize, ULONG cmdBufSize)
{
    ...
    if (InterlockedCompareExchange(&g_KiqInitGuard, 1, 0) != 0) {
        ...
        /* Timeout: first thread may have failed — don't spin forever */
        while (!g_KiqRingInitialized && waitStall < 5000000) {
            KeStallExecutionProcessor(1);
            waitStall++;
        }
        if (!g_KiqRingInitialized && g_KiqInitGuard == 0) {
            return STATUS_DEVICE_NOT_READY;
        }
    ...
    KeInitializeMutex(&g_KiqRingLock, 0);
    g_KiqRingWptr = 0;
    g_KiqRingInitialized = TRUE;

    /* Ensure GPU proxy is available before programming HW registers */
    if (!g_GpuProxyAvailable) {
        KdPrint(("KIQ: No GPU proxy, trying proxy init\n"));
        NTSTATUS proxyStatus = PspGpuProxyInit(devExt);
        if (!NT_SUCCESS(proxyStatus)) {
...
NTSTATUS PspKiqSubmit(PDEVICE_EXTENSION devExt, PPSP_KIQ_SUBMIT_REQUEST req)
{
    ...
    /* KMUTEX: proxy reads need PASSIVE — not spinlock across Zw */
    (void)KeWaitForSingleObject(&g_KiqRingLock, Executive, KernelMode, FALSE, NULL);
    ...
    /* Check ring capacity — mask RPTR to 9 bits (KIQ_WPTR is 9-bit, max 512 DWORDs) */
    {
        ...
        if (used + req->CommandCount > ringDwords) {
            KeReleaseMutex(&g_KiqRingLock, FALSE);
            PspGpuProxyWriteRegister(GPU_GRBM_GFX_INDEX, GRBM_GFX_INDEX_BROADCAST);
...
{
    /* Mutex: proxy Zw + MmFree need PASSIVE — not spinlock */
    (void)KeWaitForSingleObject(&g_KiqRingLock, Executive, KernelMode, FALSE, NULL);
    g_KiqRingInitialized = FALSE;

    /* Halt CP engines via GPU proxy (never direct BAR5) */
    if (g_GpuProxyAvailable || g_GpuDriverHandle != NULL) {
...
NTSTATUS PspGpuPm4Submit(PDEVICE_EXTENSION devExt, PPSP_GPU_PM4_SUBMIT_REQUEST req, PPSP_GPU_PM4_SUBMIT_RESPONSE resp)
{
    ...
    /* Write PM4 commands to ring — KMUTEX (proxy read under lock) */
    (void)KeWaitForSingleObject(&g_KiqRingLock, Executive, KernelMode, FALSE, NULL);
    wptr = g_KiqRingWptr;
