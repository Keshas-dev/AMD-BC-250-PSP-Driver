...
- **Root cause GPU INIT deadlock fixed:** full-init held `DeviceMutex` across `DreamV3HwInitialize` → PSP `GET_GPU_INFO` → GPU `0x900` re-acquire (non-recursive FastMutex) → permanent hang. Fix: release/re-acquire around full init + `HwInitInProgress` re-entry guard; NBIO_MAP path skips KiqInit entirely.
- **Code Reviewer APPROVE** (GPU paths balanced: braces 1104/1104, parens 4230/4230; header 19/19, 95/95).

### Verified test matrix (2026-09-23, ALL PASS)
| Layer | Tool | Result |
|-------|------|--------|
| GPU INIT NBIO_MAP | `gpu-init-explicit.exe` | `INIT OK br=32`, GPU_ID=`0x9FFF9700`, GRBM=`0`, **exits clean** |
| GPU INIT full Flags=0 | `full-init-test.exe` | `SUCCESS — no TDR`, `Step_HwInit=11`, **exits clean** |
| PSP status | `test-psp-driver.exe -s` | Alive YES, C2PMSG_64=`0x80000000`, C2PMSG_81=`0x002C7A89`, GRBM UNLOCKED |
| PSP GPU bridge | (in spi-pg test) | MmioVA=`0xF8934000` (own BAR0), skip dual-map, SIG1=`0xFEDCBAEF` |
| SPI_PG read | `spi-pg-nbio-test.exe -r` | **SPI_PG=0 [gated]** (expected SOS-lock); NBIO re-unlock gle=31 (SIGs already at boot); SMU Q0 OK |
| SMU mailbox | `bar5-smn-test.exe` | SMU 88.6.0, 1500MHz, Features `0xDD602C7D` |
| SMU CPU msgs | `smu-all-msgs-test.exe` | **16/16 PASS** (7 read + 9 write, no wedge) |
| PSP GPCOM ring | `psp-ring-submit-test.exe` | INIT Result=1, GET_FW_ATTESTATION SUCCESS, WPTR advances |

### Architecture (unchanged, reconfirmed)
- **PSP `1022:143E` BAR0 `0xFE700000`** = PSP owner only; **never** `MmMapIoSpace(0xFE800000)`.
- All PSP→GPU MMIO = raw proxy IOCTLs **`0x900`/`0x901`** under GPU `DeviceMutex`.
- Install order: **GPU first**, then PSP.
- PSP NBIO unlock best-effort only — SPI_PG gate remains SOS (higher privilege); `PROG_REG (0x0B)` = NOT_IMPLEMENTED on BC-250 SOS.

### Build/install notes (2026-09-23)
- GPU: `build.bat` (F: VS2022+WDK auto-detect), Code Reviewer before build, sign SHA1 `34AFF96C...`.
- Device Manager install preferred this session (test-sign rank loses to Microsoft inbox); uninstall w/ Delete → reboot → browse `output\` → reboot.
- Compile tests: F: vcvars64 + WDK 10.0.26100.0 includes/libs (old E: paths dead).
- Brace balance checker: rankinis PowerShell scanner; no CI/lint.

### Committed 2026-09-23
- GPU `AGENTS.md` + `README.md` (deadlock fix + test matrix).
- PSP `AGENTS.md` + `README.md` (coexistence verification + architecture).

### Remaining
- `PspKiq.c` lines ~57/60/258 still *log* `g_Bar5Mapping`/`GpuMmioBase` in conditions — falls through to proxy when handle set; optional cleanup only.
- Brace balance verified: `PspDriver.c` 222/222, `PspCore.c` 86/86, `PspSmu.c` 11/11, `PspKiq.c` 72/72, GPU `kmd.c` 1115/1115.
- Optional: `spi-pg-nbio-test.exe w` — SPI_PG still 0 expected (SOS gate).
- Crash-4 dump `092326-9312-01.dmp` **missing**; check `C:\Windows\MEMORY.DMP` via Admin script if 0x1E returns.

**User decision (LT):** "psp only atrakina ka mums reik o gpu dream draiveris tvarkosi su savo registrais" — PSP unlocks via its own window/proxy; **atikmdag exclusively owns GPU BAR5 `0xFE800000`**.

### Hardware windows (two different PCI devices)
| Device | BDF | BAR | Owner |
|--------|-----|-----|-------|
| AMD GCN GPU | `1002:13FE` | BAR5 `0xFE800000` (512KB) | **GPU driver only** (`DeviceMutex` serialized) |
| AMD PSP/CCP | `1022:143E` | BAR0 `0xFE700000` (1MB) + `0xFE884000` (8KB) | **PSP driver** (pa_v1 C2PMSG_28..30 future unlock path) |

### A/B test (definitive, 2026-09-23)
- PSP installed → **4× bugcheck 0x1E** (~1h), fault always `nt+0x6B3257` iretq, Arg4=`0xFFFFFFFFFFFFFFFF`
- PSP removed + reboot → FurMark + `spi-pg-nbio-test` clean
- **Conflict:** PSP `IOCTL_PSP_INIT_HW` PA=`0xFE800000` dual-mapped GPU BAR5 into `g_Bar5Mapping` (bypassed GPU `DeviceMutex`); both drivers wrote NBIO sigs `0xC100`/`0xC180`; dual `HalSetBusDataByOffset` Command|=0x7 on 1022:143E

### Rules (MUST follow)
1. **NEVER `MmMapIoSpace(0xFE800000)`** in PSP driver — not in `DriverEntry`, `INIT_HW`, `BOOT_SEQ`, or anywhere.
2. **All GPU register access = GPU proxy** `PspGpuProxyReadRegister` / `PspGpuProxyWriteRegister` → raw IOCTLs **`0x900`/`0x901`** (NOT `CTL_CODE`) on `\Device\AMDBC250DreamV43`.
3. GPU driver cases `0x900`/`0x901` are wrapped in **`ExAcquireFastMutex(&DevExt->DeviceMutex)`** — proxy serializes with GPU's own MMIO.
4. Legacy dual-map: if `GpuMmioBase != NULL`, unmap + clear `g_Bar5Mapping`/`g_Bar5Size` under `g_Bar5MappingLock`.
5. `PspAutoInitialize` maps **only PSP BAR0** (`0xFE700000`), requires `Bar0Base`, best-effort `PspGpuProxyInit`.
6. Init/probe order: install **GPU first**, then PSP; `PspEnablePciMemory()` enables Command bit1 on `1022:143E` pre-map.
7. Globals still exist for compatibility but **must stay NULL as live maps**: `g_Bar5Mapping`, `devExt->GpuMmioBase` (cleanup only).

### Code changed 2026-09-23 (proxy conversion)
| File | What |
|------|------|
| `PspCore.c` | `PspAutoInitialize` no GPU map + release legacy; `PspGpuProxyRead/WriteRegister` **proxy-only** (no `g_Bar5Mapping` direct path); `PspSendMailboxCommand`; `PspLoadIpFwViaMailbox` |
| `PspDriver.c` | `PSP_READ/WRITE_MAILBOX` macros → proxy; auto-init gate = `Bar0Base`/`MmioBase`; `INIT_HW` else-branch proxy-only; `NBIO_UNLOCK`; `PspDoBootSequence` NBIO+GRBM; `BOOT_SEQUENCE` no dual-map; `PROBE`; `GET_STATUS`; `GET_GPU_INFO`; `REG_PROG`; `LOAD_TOC`; `READ_REG`/`WRITE_REG` GPU→proxy; `PspSendSmcBoot` |
| `PspSmu.c` | Gate = `g_GpuProxyAvailable` / `g_GpuDriverHandle` (SmnRead/Write already proxy) |
| GPU `amdbc250_dream_kmd.c` | `0x900`/`0x901` under `DeviceMutex` |

### Related external intel (2026-09-23)
- Linux `pspv_bc250` BAR2 offsets: cmdresp `0x10544`, bootloader `0x109EC`=`0x001C0102`, feature `0x109FC`=2, inten `0x10690`; **pa_v1 C2PMSG_28..30** platform mailbox (no TEE wait).
- PSP ring `PROG_REG (0x0B)` = NOT_IMPLEMENTED on BC-250 SOS; SPI_PG gate is SOS-locked (higher privilege) — NBIO unlock is best-effort only.
- GPU driver comment (`amdbc250_psp.c:1216`): NBIO firewall blocks `0xC000-0xCFFF` from all host paths anyway.
- **`simpmix/bc250-encoding-decoding-fix` (2026-09-23):** VCN 2.0.3 permanently locked by PSP_BL **SEC_GASKET~0x24** (926 pre-x86 writes, ~816 DF ACL `0x09xxxxxx`) — host `0xB8/0xBC`→0xFFFFFFFF, SMU wedge, GPU regs dropped; latches `CC_UVD_HARVESTING=0x3` @ SMN **`0x1f81c`** + `[0x1f820]=0x00185103`. ⚠️ **NEVER read SMN `0x1f81c`** (instant DF hang, AC cycle). No `vcn_2_0_3.bin` (Alex Deucher). Encode path = Vulkan compute 40 CUs; audio fix DCCG `0x05E0/E4/E8`. Author Mattia Tadini = LKML PSP patches.
