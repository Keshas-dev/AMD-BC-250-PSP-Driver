...
```
0. Poll C2PMSG_64 bit 31 (MBOX_TOS_READY_FLAG) until set  ← **CRITICAL** added in v2
...
Timeout: 10s for step 0 (TOS_READY), 3s for step 5 (ring response). Both poll at 1ms intervals.
...
- `PspCreateRing()` (IOCTL_PSP_CREATE_RING) now **waits for C2PMSG_64 bit 31** (MBOX_TOS_READY_FLAG) before writing ring registers
- Timeout: 10 seconds, poll every 1ms. If timeout, return `STATUS_TIMEOUT`
- After writing ring regs, **waits for response bit 31** (MBOX_TOS_RESP_FLAG) with 3s timeout
- Mailbox CMD 0x4/0x8 path kept for backward compatibility but may be ineffective
